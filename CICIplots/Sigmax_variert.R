...
load("C:/Users/bruger/OneDrive/Skrivebord/Speciale/Speciale-git/data1001/sigmax_varieret_100.RData")
poOLD <- parms_org
sdOLD <- sd_org
load("C:/Users/bruger/OneDrive/Skrivebord/Speciale/Speciale-git/data1001/TMB_sigmax_varieret_100.RData")
poTMB <- parms_org
sdTMB <- sd_org
...
mtext("MU when sigma x is changed",side=3,outer=TRUE, line=-1,cex.main=1.5)
...
title("For TMB",cex.main=0.8)
mtext("Theta when sigma x is changed",side=3,outer=TRUE, line=-1,cex.main=1.5)

#lookign at sig x
par(mfrow=c(2,1))
CICI(poOLD,isx[2],sdOLD,int_sig_x,log(int_sig_x))
title("For Old pack",cex.main=0.8)
CICI(poTMB,isx[1],sdTMB,int_sig_x,log(int_sig_x))
title("For TMB",cex.main=0.8)
mtext("Sigma x when sigma x is changed",side=3,outer=TRUE, line=-1,cex.main=1.5)

#looking at sigy
par(mfrow=c(2,1))
CICI(poOLD,isy[2],sdOLD,int_sig_x,rep(log(1e-2),10))
title("For Old pack",cex.main=0.8)
CICI(poTMB,isy[1],sdTMB,int_sig_x,rep(log(1e-2),10))
title("For TMB",cex.main=0.8)
mtext("Sigma y when sigma x is changed",side=3,outer=TRUE, line=-1,cex.main=1.5)
