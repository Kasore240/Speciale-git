...
load("C:/Users/bruger/OneDrive/Skrivebord/Speciale/Speciale-git/true data/sigmax_varieret_1000.RData")
poOLD1 <- parms_org
sdOLD1 <- sd_org
...
plot(int_sig_x ,apply(exp(poOLD[,2,]),2,sd,na.rm=TRUE),pch=15, col="red",ylim=c(0,0.02),ylab = expression(paste('Sd of estimated vaue of ',bold(sigma[y]))),xlab = expression(paste('Size of ', sigma[x])))
legend("topleft",c('Ctsmr','CtsmrTMB'),pch=c(15,8),col=c('red','blue'))
...
CICI(poOLD,imu[2],sdOLD,int_sig_x,1,10)
title("Ctsmr",cex.main=0.8)
CICI(poTMB,imu[1],sdTMB,int_sig_x,1,10) 
...
#looking at theta
#par(mfrow=c(2,1))
CICI(poOLD,itheta[2],sdOLD,int_sig_x,10,10)
title("Ctsmr",cex.main=0.8)
CICI(poTMB,itheta[1],sdTMB,int_sig_x,log(10),10)
title("CtsmrTMB",cex.main=0.8)
mtext(expression(paste(theta, " when size of ", sigma[x], " is changed")),side=1,outer=TRUE, line=-18,cex.main=1.5)

#lookign at sig x
par(mfrow=c(2,2))
CICI(poOLD,isx[2],sdOLD,int_sig_x,log(int_sig_x),20)
title("Ctsmr",cex.main=0.8)
CICI(poTMB,isx[1],sdTMB,int_sig_x,log(int_sig_x),20)
title("CtsmrTMB",cex.main=0.8)
mtext(expression(paste(sigma[x], " when size of ", sigma[x], " is changed")),side=3,outer=TRUE, line=-1.5,cex.main=1.5)

#looking at sigy
#par(mfrow=c(1,2))
CICI(poOLD,isy[2],sdOLD,int_sig_x,log(1e-2),20)
title("Ctsmr",cex.main=0.8)
CICI(poTMB,isy[1],sdTMB,int_sig_x,log(1e-2),20)
title("CtsmrTMB",cex.main=0.8)
