...
source("C:/Users/bruger/OneDrive/Skrivebord/Speciale/Speciale-git/Funktioner/sim_OU_sqrt_EM.R")
...
OU_sqrt_ctsmr <- function(init_pars,init_lb,init_ub){
  ...
  model$addSystem( dx ~ theta * (mu-sqrt(x)) * dt  + exp(log_sigma_x)*dw)
...
OU_sqrt_ctsmrTMB <- function(init_pars,init_lb,init_ub){
 ...
 # obj$set_modelname("ornstein_uhlenbeck_sqrt")
  ...
  # Add system equations
  obj$add_systems(
    dx ~ theta * (mu-sqrt(x)) * dt + sigma_x*dw
  )
...
fitTMB <- obj$estimate(.data, method="ekf", use.hessian=T,ode.timestep = 1e-3)
