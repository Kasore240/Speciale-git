...
template<class Type>
matrix<Type> dfdx__(Type logtheta, Type mu, Type x){
	matrix<Type> ans(1,1);
	ans(0,0) = exp(logtheta) * (mu - 2 * x);
	return ans;
}
...
//////////// UKF METHOD ///////////
	 ...
	 for(int i=0 ; i<t.size()-1 ; i++){
		 ...
		 //////////// Time-Update ///////////
		 for(int j=0 ; j<N__(i) ; j++){
			 F__  = construct_F__(Xsp0__,logtheta, mu);
			 G__  = g__(logsigma_x);
